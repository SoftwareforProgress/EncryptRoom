...
- Password-protected invites do not embed plaintext room secrets; they store password-wrapped room config (`Argon2id` + `ChaCha20-Poly1305`).
...
- Client supports `tls://` relay transport with certificate verification, and relay supports static certs or Let's Encrypt autocert.
...
- Network-level observers can still see that a client connects to the relay (especially if you choose plaintext `tcp://` transport).
